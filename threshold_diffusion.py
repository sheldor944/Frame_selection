...
def parse_arguments():
    ...
    parser.add_argument('--target_avg_frames', type=int, default=16,
                        help='Target number of frames for calculating threshold')
    parser.add_argument('--use_threshold', action='store_true',
                        help='Enable threshold filtering based on target_avg_frames')
...
class KeyframeSelector:
    ...
    def select_keyframes(self, max_frames: int, target_frames_for_threshold: int = None, 
                         use_threshold: bool = False) -> List[int]:
        """
        Select keyframes using greedy NMS strategy with optional threshold filtering.
        
        Args:
            max_frames: Maximum number of frames to select
            target_frames_for_threshold: Number of top frames to use for calculating threshold
            use_threshold: Whether to apply threshold filtering
        ...
        # Calculate threshold if enabled
        threshold = 0.0
        if use_threshold and target_frames_for_threshold is not None:
            # Get top target_frames_for_threshold scores
            top_k = min(target_frames_for_threshold, len(self.diffused_scores))
            top_scores = np.partition(self.diffused_scores, -top_k)[-top_k:]
            avg_top_scores = np.mean(top_scores)
            threshold = avg_top_scores / 2.0
            print(f"  → Threshold enabled: avg of top {top_k} = {avg_top_scores:.4f}, threshold = {threshold:.4f}")
...
def process_video(scores: List[float], frame_ids: List[int],
    ...
    selected_frames = selector.select_keyframes(
        ...
        target_frames_for_threshold=args.target_avg_frames if args.use_threshold else None,
...
def build_output_filename(args):
    if args.use_threshold:
        name = (
            ...
            f"thresh{args.target_avg_frames}_"
...
def main(args):
    ...
    if args.use_threshold:
        print(f"  MODE: Threshold filtering enabled (based on top {args.target_avg_frames} frames)")
    print("=" * 60)
    print(f"Dataset: {args.dataset_name}")
    print(f"Feature Model: {args.extract_feature_model}")
    print(f"Max Frames: {args.max_num_frames}")
    if args.use_threshold:
        print(f"Threshold Calculation: avg(top {args.target_avg_frames}) / 2")
