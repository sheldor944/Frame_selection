...
class VideoMMEMetadataOrganizer:
    """
    Organizes VideoMME metadata by task_type and manages folder structure.
    Also supports organization by duration, domain, or sub_category.
    """
    
    def __init__(self, video_metadata_path: str, organize_by: str = 'task_type'):
        """
        Initialize with the VideoMME metadata JSON.
        
        Args:
            video_metadata_path: Path to JSON file with VideoMME metadata
            organize_by: Field to organize by ('task_type', 'duration', 'domain', 'sub_category')
        """
        ...
        print(f"📁 Found {len(self.categories)} unique {organize_by} values:")
        for cat, videos in sorted(self.category_mapping.items()):
    ...
    def _build_category_mapping(self) -> Dict[str, List[dict]]:
        """
        Build a mapping of organize_by field -> list of video entries.
        """
        category_map = defaultdict(list)
        
        for idx, entry in enumerate(self.video_metadata):
            # Get category based on organize_by field
            category = entry.get(self.organize_by, 'unknown')
    ...
    def get_entry_by_question_id(self, question_id: str) -> Optional[dict]:
        """Find a video entry by its question_id (e.g., '001-1')."""
        for entry in self.video_metadata:
    ...
    def print_metadata_summary(self):
        """Print a summary of all metadata fields and their unique values."""
        print("\n" + "=" * 80)
        print("📊 VIDEOMME METADATA SUMMARY")
        print("=" * 80)
        
        fields_to_summarize = ['task_type', 'duration', 'domain', 'sub_category']
        
        for field in fields_to_summarize:
            values = self.get_unique_values(field)
            print(f"\n{field}:")
            for val in values:
                count = sum(1 for e in self.video_metadata if e.get(field) == val)
                print(f"   • {val}: {count}")
        
        print("\n" + "=" * 80)


class VideoMMEMultiAlgorithmVisualizer:
    def __init__(self, metadata_path: str, scores_path: str, frames_path: str, 
                 ...
                 output_dir: str = "videomme_comparison_plots",
                 organize_by: str = 'task_type',  # Can be 'task_type', 'duration', 'domain', 'sub_category'
                 n_workers: Optional[int] = None,
                 max_memory_percent: int = 80):
        """
        Memory-safe visualizer for VideoMME with task_type-based folder organization.
        ...
        Args:
            ...
            video_metadata_path: Path to VideoMME metadata JSON with task_type
            output_dir: Parent output directory
            organize_by: Field to organize folders by ('task_type', 'duration', 'domain', 'sub_category')
            n_workers: Number of parallel workers
            max_memory_percent: Stop if RAM exceeds this percentage
        """
        print("=" * 80)
        print("🚀 VIDEOMME MULTI-ALGORITHM VISUALIZER WITH TASK TYPE ORGANIZATION")
        print("=" * 80)
        ...
        # Initialize VideoMME metadata organizer
        print("\n📥 Loading VideoMME metadata...")
        self.organize_by = organize_by
        self.video_organizer = VideoMMEMetadataOrganizer(video_metadata_path, organize_by)
        
        # Print metadata summary
        self.video_organizer.print_metadata_summary()
        
        # Create parent output directory
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True, parents=True)
        
        # Create category subdirectories based on organize_by field
        print(f"\n📁 Creating {organize_by} folders...")
        self.category_dirs = {}
    ...
    def _build_doc_id_to_category_map(self):
        """
        Build a mapping from doc_id (index in metadata) to task_type/category.
        
        VideoMME uses question_id like '001-1', '001-2' etc.
        """
        self.doc_id_to_category = {}
        self.doc_id_to_video_meta = {}
        
        # Try to match by question_id or video_id
        for doc_id, meta in enumerate(self.metadata):
            # Get identifiers from original metadata
            question_id = meta.get('question_id', '')
            video_id = meta.get('video_id', '')
            
            # Try to find matching entry in VideoMME metadata
            matched_entry = None
            
            # Try matching by question_id first (most specific)
            if question_id:
                matched_entry = self.video_organizer.get_entry_by_question_id(question_id)
            ...
            if matched_entry:
                category = matched_entry.get(self.organize_by, 'unknown')
        ...
        print(f"\n📊 Doc ID to {self.organize_by} Mapping:")
    ...
    def _plot_single_video(self, doc_id: int, show_top_k: bool = True, 
        ...
        """
        Memory-optimized plotting for a single video with task_type-based output.
        """
        try:
            ...
            ax.set_title(f'ALL ALGORITHMS OVERLAY | Task Type: {category}', 
            ...
            # Get metadata for title - VideoMME specific fields
            video_id = meta.get('video_id', 'N/A')
            question_id = meta.get('question_id', 'N/A')
            
            # Get additional info from video_meta if available
            if video_meta:
                duration = video_meta.get('duration', 'N/A')
                domain = video_meta.get('domain', 'N/A')
                sub_category = video_meta.get('sub_category', 'N/A')
                task_type = video_meta.get('task_type', 'N/A')
            else:
            ...
            suptitle += f'Task: {task_type} | Domain: {domain} | Duration: {duration} | SubCat: {sub_category}'
            fig.suptitle(suptitle, fontsize=16, fontweight='bold', y=0.995)
            
            plt.tight_layout(rect=[0, 0, 1, 0.99])
            
            # Generate filename and get category-based output path
            safe_question_id = self._sanitize_folder_name(str(question_id))
            filename = f"doc_{doc_id:04d}_v{video_id}_q{safe_question_id}_comparison.png"
    ...
    def generate_all_plots(self, video_ids: Optional[List[int]] = None, 
                          ...
                          task_types: Optional[List[str]] = None,  # Filter by task_type
                          durations: Optional[List[str]] = None,   # Filter by duration
        ...
        """
        Memory-safe generation with checkpointing, batching, and multiple filters.
        
        Args:
            video_ids: Specific video IDs to process
            max_videos: Max number of videos
            task_types: List of task_types to process (None = all)
            durations: List of durations to filter ('short', 'medium', 'long')
        ...
        """
        print("=" * 80)
        print("🚀 VIDEOMME MULTI-ALGORITHM VISUALIZATION")
        print("=" * 80)
        print(f"📁 Output: {self.output_dir.absolute()}")
        print(f"🔧 Algorithms: {', '.join(self.algorithms.keys())}")
        print(f"📂 Organizing by: {self.organize_by}")
        ...
        if video_ids is not None:
            ...
            if task_types is not None:
                print(f"\n🔍 Filtering by task_types: {task_types}")
                to_process = [
                    doc_id for doc_id in to_process
                    if self.doc_id_to_video_meta.get(doc_id, {}).get('task_type') in task_types
                ]
            
            if durations is not None:
                print(f"🔍 Filtering by durations: {durations}")
                to_process = [
                    doc_id for doc_id in to_process
                    if self.doc_id_to_video_meta.get(doc_id, {}).get('duration') in durations
                ]
            ...
            if domains is not None:
                ...
                to_process = [
                    doc_id for doc_id in to_process
                    if self.doc_id_to_video_meta.get(doc_id, {}).get('domain') in domains
                ]
        ...
        print(f"\n📊 Videos to process by {self.organize_by}:")
    ...
    def generate_category_report(self):
        ...
        print("\n" + "=" * 80)
        print(f"📂 {self.organize_by.upper()} ORGANIZATION REPORT")
        print("=" * 80)
        ...
        for category in sorted(self.video_organizer.get_all_categories()):
            ...
            if len(videos) > 0:
                # Sample question IDs
                sample_ids = [v.get('question_id', 'N/A') for v in videos[:5]]
                print(f"   Sample question IDs: {', '.join(sample_ids)}")
                
                # Show domain distribution within this task_type
                domain_counts = defaultdict(int)
                for v in videos:
                    domain_counts[v.get('domain', 'unknown')] += 1
                print(f"   Domain distribution: {dict(domain_counts)}")
        
        print("\n" + "=" * 80 + "\n")
    
    def generate_cross_category_analysis(self):
        """Generate a cross-category analysis showing task_type vs domain vs duration."""
        print("\n" + "=" * 80)
        ...
        for entry in self.video_organizer.video_metadata:
            ...
            cross_tab[(task_type, domain)][duration] += 1
        
        print("\nTask Type × Domain × Duration:")
        print("-" * 80)
        
        for (task_type, domain), duration_counts in sorted(cross_tab.items()):
            total = sum(duration_counts.values())
            duration_str = ", ".join([f"{d}: {c}" for d, c in sorted(duration_counts.items())])
            print(f"  {task_type} | {domain}: {total} total ({duration_str})")
...
if __name__ == "__main__":
    ...
    # ========================================================================
    # CONFIGURATION FOR VIDEOMME
    # ========================================================================
    
    # Original data paths (your existing files)
    METADATA_PATH = "/home/train01/aks/Frame_selection/single_algo_tester/vmme/metadata_vmme.json"  # Your original metadata
    SCORES_PATH = "/home/train01/aks/Frame_selection/outscores/videomme/blip/scores.json"       # Your scores file
    FRAMES_PATH = "/home/train01/aks/Frame_selection/outscores/videomme/blip/frames.json"  # Your frames file
    
    # NEW: Path to VideoMME metadata with task_type, duration, domain, etc.
    VIDEO_METADATA_PATH = "/home/train01/aks/Frame_selection/THESIS_VMME/selected_tmas_videomme_blip_k32_auto_budget_based_curvgrad_normmax_clip95_hybrid_cov1.73_opt.json"  # Your new JSON file with task_type
    
    # Algorithm selections
    ALGORITHM_SELECTIONS = {
        "AKS": "/home/train01/aks/Frame_selection/THESIS/Thesis_aks/videomme/selected_videomme_blip_TOP_k32_ratio1.json",
        "TMAS_budget": "/home/train01/aks/Frame_selection/THESIS/videomme/selected_tmas_videomme_blip_k32_auto_budget_based_curvgrad_normmax_clip95_hybrid_cov1.73_opt.json",
        "TMAS_half_life": "/home/train01/aks/Frame_selection/THESIS/videomme/selected_tmas_videomme_blip_k32_auto_half_life_curvlapl_normmax_clip95_hybrid_cov1.73_opt.json"
    }
    
    # Parent output directory - subfolders will be created per task_type
    OUTPUT_DIR = "videomme_plots_by_task_type_THESIS"
    
    # ========================================================================
    # ORGANIZE BY OPTIONS:
    # - 'task_type': Counting Problem, Information Synopsis, etc.
    # - 'duration': short, medium, long
    # - 'domain': Knowledge, etc.
    # - 'sub_category': Humanity & History, etc.
    # ========================================================================
    
    ORGANIZE_BY = 'task_type'  # Change this to organize by different fields
    ...
    try:
        visualizer = VideoMMEMultiAlgorithmVisualizer(
            ...
            organize_by=ORGANIZE_BY,  # Organize by task_type
            n_workers=32,
        ...
        visualizer.generate_cross_category_analysis()
        
        # ====================================================================
        # GENERATION OPTIONS
        # ====================================================================
        
        # OPTION 1: Process ALL videos (organized by task_type)
        ...
        # OPTION 2: Process only specific task_types
        # visualizer.generate_all_plots(
        #     task_types=['Counting Problem', 'Information Synopsis'],
        #     parallel=True,
        #     dpi=200,
        #     batch_size=100,
        #     resume=True
        ...
        # OPTION 3: Process only short videos
        # visualizer.generate_all_plots(
        #     durations=['short'],
        #     parallel=True,
        #     dpi=200,
        ...
        # OPTION 4: Process specific domain
        # visualizer.generate_all_plots(
        #     domains=['Knowledge'],
        #     parallel=True,
        #     dpi=200,
        ...
        # OPTION 5: Combine filters - short Knowledge videos with Counting Problem
        # visualizer.generate_all_plots(
        #     task_types=['Counting Problem'],
        #     durations=['short'],
        #     domains=['Knowledge'],
        #     parallel=True,
        #     dpi=200,
        ...
        # OPTION 6: Test with small batch
        # visualizer.generate_all_plots(
        #     max_videos=10,
        #     parallel=False,
        #     dpi=150,
        ...
        print(f"\n\n❌ FILE NOT FOUND: {e}")
        print("💡 Make sure all data files exist and paths are correct")
    except MemoryError:
        print("\n\n❌ OUT OF MEMORY ERROR")
