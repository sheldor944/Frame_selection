...
def parse_arguments():
    ...
    parser.add_argument('--tmas_auto_coverage', type=float, default=1.73,
    ...
    # ═══════════════════════════════════════════════════════════════════════
    parser.add_argument('--tmas_decay_mode', type=str, default='budget_based',
...
class TemporalMemorySuppressionCalculator:
    """
    Temporal Memory-Aware Suppression (TMAS) Calculator with global tracking.
    """
    ...
    def get_current_radius_for_position(self, candidate_idx: int) -> float:
        """
        Calculate current suppression radius at candidate position based on ALL selected frames.
        Uses global selection map to track decay.
        ...
        for selected_idx, selection_num in self.selection_map.items():
            ...
            if self.config.decay_mode == 'budget_based':
                # Distance = number of frames selected after this one
                frames_since = self.selection_counter - selection_num
                decay_distance = float(frames_since)
            else:
                # Distance = temporal frame distance
                decay_distance = abs(candidate_idx - selected_idx)
            
            # Calculate suppression radius from this selected frame
            if self.config.mode == 'auto':
...
class KeyframeSelector:
    """
    Selects keyframes using sorted greedy selection with TMAS.
    """
    ...
    def select_keyframes(self, max_frames: int, optimize_remaining: bool = False) -> List[int]:
        """
        Select keyframes in sorted order with TMAS suppression.
        
        Process:
        1. Sort all frames by score (highest first)
        2. Pick highest scored frame
        3. Suppress nearby frames based on current radius
        4. Decay radius according to decay mode
        ...
        # Phase 1: TMAS-guided sorted selection
    ...
    def _sorted_tmas_selection(self, max_frames: int) -> List[int]:
        """
        Perform sorted selection with TMAS suppression.
        ...
        sorted_candidates = sorted(
            ...
            key=lambda x: (-x[0], x[1])  # Sort by score desc, then index
        )
        ...
        for score, idx in sorted_candidates:
            ...
            # Notify TMAS calculator (updates radius)
            if self.tmas_calculator:
...
def main(args):
    ...
    print("=" * 80)
    print("🚀 TMAS Frame Selection (No Diffusion)")
    print("=" * 80)
